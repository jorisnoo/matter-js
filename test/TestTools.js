...
const comparisonReport = (capturesDev, capturesBuild, devSize, buildSize, buildVersion, save, benchmark) => {
    ...
    const captureSummary = Object.entries(capturesDev)
        .map(([name]) => {
            const changedIntrinsics = !deepEqual(capturesDev[name].intrinsic, capturesBuild[name].intrinsic);
...
const importUncached = async (modulePath) => {
    const resolved = require.resolve(modulePath);
    const url = (await import('url')).pathToFileURL(resolved).href;
...
const toMatchIntrinsics = {
    toMatchIntrinsics(currentCaptures, referenceCaptures) {
        ...
        entries.forEach(([name, current]) => {
            ...
            if (!deepEqual(current.intrinsic, reference.intrinsic)) {
                changed = true;
            }
        });
